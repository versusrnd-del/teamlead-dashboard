Это **блестящее продуктовое решение**! Ты сейчас нащупал классическую проблему метрик: когда инженер делает подвиг и разгребает завалы полугодовой давности, система его «наказывает» огромным Cycle Time, и на дашборде он выглядит как аутсайдер.

Мы сделаем так: ИИ будет находить закрытые задачи, чей возраст превышает **30 дней**, и бережно собирать их в отдельный массив `techDebtClosed` (Закрытый техдолг). А на дашборде под цифрой Cycle Time появится стильный фиолетовый бейдж (например, `+2 стар.`). При наведении на него всплывет подсказка: какие именно «древние» тикеты раскопал и закрыл админ, и сколько дней они висели.

### 1. Обновленный Промпт для Задач (Инфраструктура)
Я добавил в **ШАГ 4** сбор старых задач. Скопируй этот промпт для следующего анализа:

```markdown
Ты — Senior Data Analyst и Agile Coach инфраструктурной команды. Я загрузил выгрузку ЗАДАЧ из Jira (формат CSV). В ней есть закрытые задачи за неделю и весь открытый бэклог.

ВАЖНОЕ ПРАВИЛО 1 (АНАЛИЗ ДАННЫХ): ОБЯЗАТЕЛЬНО используй среду выполнения Python (Advanced Data Analysis / Code Interpreter). Напиши скрипт, прочитай CSV, посчитай все переменные строго математически кодом и только потом формируй JSON. Никаких подсчетов "в уме"!
ВАЖНОЕ ПРАВИЛО 2: В Jira компоненты могут выгружаться в несколько столбцов ("Компоненты", "Компоненты_(1)" и т.д.). В коде обязательно склей эти столбцы в один (например, `df['All_Tags']`).
ВАЖНОЕ ПРАВИЛО 3: В текстовых выводах КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО использовать одиночный обратный слеш (\). Заменяй виндовые пути на прямые.

ШАГ 1: ЖЕСТКАЯ КЛАССИФИКАЦИЯ (ЧЕРЕЗ PYTHON)
В Jira названия статусов могут меняться. ЖЕЛЕЗОБЕТОННЫЙ признак закрытой задачи — это наличие даты в колонке "Дата решения". В коде pandas ВООБЩЕ НЕ ИСПОЛЬЗУЙ колонку 'Статус' для определения закрытых задач! 

Сначала жестко обработай пустые значения:
`df['Дата решения'] = df['Дата решения'].astype(str).replace(['nan', 'NaT', 'None', 'NAT', 'NaN'], '').str.strip()`

1. Группа "ВЫПОЛНЕНО" (df_completed): `df[df['Дата решения'] != '']`
2. Группа "В ОЧЕРЕДИ" (df_open): `df[df['Дата решения'] == '']`

ШАГ 2: FLOW METRICS И БАЗОВЫЕ РАСЧЕТЫ
* inflowThisWeek: Количество ВСЕХ задач с датой "Создано" на текущей неделе.
* urgentCompleted: Количество задач в df_completed, где `All_Tags` содержит "Срочная линия".
* sprintCompleted: Количество ВСЕХ ОСТАЛЬНЫХ задач в df_completed. (Математически: `len(df_completed) - urgentCompleted`).
* urgentQueue: Количество в df_open, где `All_Tags` содержит "Срочная линия".
* sprintCarriedOver: Количество в df_open, где `All_Tags` содержит "Active for the week" (и не содержит "Срочная линия").
* sprintPlanned: sprintCompleted + sprintCarriedOver.
* backlog: Количество задач в df_open, где `All_Tags` содержит "OSO_Support" (и НЕ содержит "Active for the week" и "Срочная линия").
* backlogOld30: Задачи из backlog, созданные более 30 дней назад.
* backlogCompleted: 0.
* avgCycleTime: Среднее время цикла (в днях) для группы "ВЫПОЛНЕНО". Округли до 1 знака.
* reopenRate: Процент возвратов на доработку в группе "ВЫПОЛНЕНО". ИСКЛЮЧЕНИЕ: не считай возвратом системные действия (робота), если в логах есть слова "автовозврат", "защита потока", "автоматически возвращена".

ШАГ 3: АНАЛИТИКА ТРУДОЕМКОСТИ И ТИПОВ РАБОТ
* taskComplexity: Распредели df_completed по размерам (S, M, L, XL). Формат: [{"size": "...", "count": X, "description": "..."}].
* techDebtCategories: Сгруппируй backlog в 3-4 категории. Формат: [{"name": "...", "count": X}].
* taskTypesDistribution: Проанализируй теги/компоненты закрытых задач (df_completed) и выдели 4-5 главных категорий работ. Формат: [{"name": "Доступы", "percent": 40}].

ШАГ 4: ПРОФИЛИ ИНЖЕНЕРОВ, ВОЗВРАТЫ И ЗАКРЫТЫЙ ТЕХДОЛГ (TASK PERFORMERS)
Склей все столбцы с комментариями в `Full_Comments`.
Сгруппируй df_completed по колонке "Исполнитель". Для каждого собери:
"name": логин,
"closed": сколько всего закрыл,
"wip": сколько задач "В ОЧЕРЕДИ" на этом инженере,
"avgTimeMin": среднее время цикла (в днях, только число),
"commentsFreq": "Высокая/Средняя/Низкая".

СБОР ТЕХДОЛГА (РАСКОПКИ): Найди среди закрытых инженером задач те, чей Cycle Time (разница между датой решения и создания) больше 30 дней. Собери их в массив объектов `techDebtClosed`: `[{"id": "IS-123", "days": 61, "title": "Тема задачи"}]`. Если таких нет, отдай `[]`.

СБОР КАЧЕСТВА: найди задачи инженера с возвратом (в `Full_Comments` слова "возврат", "возвращаю", "не работает", "ошибка", "доработать" или статус "Переоткрыт"). 
ИСКЛЮЧЕНИЕ: Игнорируй "автовозврат", "защита потока", "автоматически возвращена".
Для КАЖДОЙ возвращенной задачи сделай выжимку "Заказчик: [...]. Админ: [...]".
Собери массив: `[{"id": "IS-123", "reason": "выжимка"}]` в `reopenedTasks`. Если нет — `[]`.
"csat": Рассчитай `5.0 - (len(reopenedTasks) / closed * 5.0)`. Округли до 1 знака. Минимальное 1.0. 

ШАГ 5: NLP-АНАЛИЗ И ВЫВОДЫ
* sprintWin: Оценка Delivery.
* sprintRisk: Узкие места процесса.
* shieldHero: Логин героя Срочных.
* blockersAndWaste: Аудит затыков (Матрица Эйзенхауэра).

ШАГ 6: АРХИВ ЗАДАЧ
`detailedTasks` из 20-30 задач. "id", "title", "created", "resolved", "cycleTime", "size", "tags", "assignee", "status", "comments".

ШАГ 7: ВЫВОД СТРОГОГО JSON
{
"inflowThisWeek": 0, "sprintPlanned": 0, "sprintCompleted": 0, "sprintCarriedOver": 0,
"urgentCompleted": 0, "urgentQueue": 0, "backlog": 0, "backlogOld30": 0, "backlogCompleted": 0,
"avgCycleTime": 0.0, "reopenRate": 0.0,
"techDebtCategories": [], "taskComplexity": [], "taskTypesDistribution": [], "taskPerformers": [],
"sprintWin": "", "sprintRisk": "", "shieldHero": "", "blockersAndWaste": "",
"detailedTasks": []
}
```
...
### 2. Обновленный код Дашборда (`App.jsx`)
Я добавил обработку массива `techDebtClosed`. Теперь в таблице задач под цифрой Cycle Time у "археологов" будет появляться фиолетовый бейдж, который при наведении покажет, что именно они закрыли.

Замени код в файле `App.jsx` на этот:
