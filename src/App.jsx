...
const generateTopProblemPostmortemReport = ({
  ...
  return `<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Постмортем ТОП-1 проблемы</title><style>
    ...
    *{box-sizing:border-box}body{margin:0;background:#eef2f7;color:var(--ink);font-family:Aptos,Calibri,"Segoe UI",Arial,sans-serif;line-height:1.48}.page{max-width:1120px;margin:0 auto;padding:34px 28px 56px}
    ...
    <header class="hero">
      <div class="hero-grid">
        ...
        <div class="meter-grid">
          <div class="pulse"><div class="meter-head"><span>SLA инцидентов</span><em>взятие + решение</em></div><div class="orb"></div><div class="pulse-title">${html(currentState.label)}</div><div class="pulse-note">${html(currentState.note)} · худший SLA недели</div><div class="sla-target"><span>Наша цель SLA</span><b>95%</b></div></div>
          <div class="phone-pulse" style="--phone:${currentPhoneState.color}"><div class="meter-head"><span>Телефонная линия</span><em>принято / пропущено</em></div><div class="phone-art"><svg class="phone-svg" viewBox="0 0 64 64" aria-hidden="true"><path d="M46.6 41.7c-2.8-1.3-5.4-1.9-7.4.5l-2.2 2.7c-5.8-3.2-10.4-7.8-13.7-13.7l2.7-2.2c2.4-2 1.8-4.6.5-7.4l-2.1-4.5c-1.2-2.6-3.5-3.9-6.1-3.1l-5.2 1.6c-2.1.7-3.6 2.7-3.4 5 .8 15.6 17.9 32.7 33.5 33.5 2.3.1 4.3-1.3 5-3.4l1.6-5.2c.7-2.6-.6-4.9-3.2-6.1l-4.5-2.2z"/></svg><div class="missed-badge">${html(Math.round(phoneMissed))}</div></div><div class="pulse-title">${html(currentPhoneState.label)}</div><div class="pulse-note">${html(currentPhoneState.note + phoneLoadSuffix)}</div><div class="sla-target"><span>Пропущено / лимит</span><b>${hasPhoneMissedData || phoneCalls > 0 ? `${html(Math.round(phoneMissed))}/${html(Math.round(phoneMissedTarget))}` : '—'}</b></div></div>
        </div>
      </div>
    ...
    <section><h2>Короткий вывод для команды</h2><div class="focus"><strong>${html(topicName)}</strong><br>Измеряем поток, SLA, маршруты и темы. На эту неделю берем одно узкое место: ${html(topicName)}. Улучшение должно быть конкретным: инструкция, права, обучение или маршрут. Эффект проверяем по SLA и доле помощи выше 1-й линии.<br><label class="modal-open" for="top-breakdown-modal">Открыть расшифровку ТОП-проблемы</label></div><div class="signal-strip"><div><b>1. Выбираем одну тему</b><span class="muted">${html(topicName)}</span></div><div><b>2. Делаем одно действие</b><span class="muted">${html(actionNeeded)}</span></div><div><b>3. Проверяем эффект</b><span class="muted">SLA, маршруты, помощь выше 1-й линии</span></div></div></section>
